       ...
       program boblqx4
!   4th and final program in Gaussian Integer suite
...
!     theorem
      ...
      common marr(200),mbarr(200),mcarr(200),mdarr(200)
       ...
       dimension mtot(100),mmul(100),ksum(100),iee(100)
       dimension nar(5000,20),littr(20),mpiv(5000,20),itex(100)
       dimension iharr(1000),larr(1000,20),n(60),jfak(30),jfreq(30)
       dimension ipd(50),mul(50),isum(50),iconar(20),mm1(20),mm2(20)
       dimension ibase(20),igg1(50),igg2(50),ibpr(50) 
       dimension iabp(50),iabpn(50),ity(50),mexp(50),lsum(50)
       dimension jbase(50),itarg(50),jtarg(50),nsum(50),ipdf(10,12)
       dimension ipdff(10),jpdff(10),ivecno(10),ibigm(10,100)
       dimension ismallm(10,100),mminv(10,100),ia0(30),ia1(30),m1(30)
       dimension m2(30)
      dimension itotarr(1500,50),nararr(1500,50),ipretx(50),mtotl(50)
       dimension igcd2(50)
      ...
      open (unit=9,file='gausparn',access='direct',form=& 
      'formatted',recl=780,status='old')
1006  format (60i4,10(12i4),10i2,10i2,10i2)
      read (9,1006,rec=1)(n(jf),jf=1,60),((ipdf(i,jf),jf=1,12),i=1,10),&
      (ipdff(jf),jf=1,10),(jpdff(jf),jf=1,10),(ivecno(jf),jf=1,10)
      ...
      mfak=0
      do i=1,10
      if (ipdf(i,2).eq.0)goto 100
      mfak=mfak+1
100   end do
      do i=1,mfak
      if (ipdf(i,2).eq.0)goto 101
      marr(1)=0
      marr(2)=1
      marr(3)=1
      do jf=1,ipdf(i,2)+2
      ...
      do jf=1,marr(2)+2
      ismallm(i,jf)=marr(jf)
      end do
101   end do
      
      do i=1,mfak
      print *,'i',i,'ismallm',(ismallm(i,jf),jf=1,ismallm(i,2)+2)
      end do
      ...
      do i=1,mfak
      if (ipdf(i,2).eq.0)goto 102
      do jf=1,mtot(2)+2
      marr(jf)=mtot(jf)
      end do
      do jf=1,ismallm(i,2)+2
      mbarr(jf)=ismallm(i,jf)
      end do
      call mendiv
      do jf=1,mdarr(2)+2
      ibigm(i,jf)=mdarr(jf)
...
102   end do
      do i=1,mfak
      print *,'i',i,'ismallm',(ismallm(i,jf),jf=1,ismallm(i,2)+2)
      print *,'i',i,'ibigm',(ibigm(i,jf),jf=1,ibigm(i,2)+2)
      print *,'i',i,'mminv',(mminv(i,jf),jf=1,mminv(i,2)+2)
      end do
...
!     n(58)=1800      
      ...
      ipd(jf)=mdarr(jf)
       ...
       open(unit=7,file='gauspar',access='direct',form=&
       'formatted',recl=514,status='old')
       read(7,6,rec=1)(ia1(jf),jf=1,10),(ia0(jf),jf=1,10),&
       (m1(jf),jf=1,10),(m2(jf),jf=1,10),irecnn,kkll,ijpow,&
       (ibase(jf),jf=1,20),&
       (jbase(jf),jf=1,20),(itarg(jf),jf=1,20),(jtarg(jf),jf=1,20),&
       kmx1,kmx2
       print *,'kkll',kkll,'kmx1',kmx1,'kmx2',kmx2
      ...
      narc5=4*(n(2)+2)*irecnn
      open (unit=2,file='gausf5',access='direct',form=&
      'formatted',recl=narc5,status='old')
       irecf=1
       read (2,2,rec=irecf)((iv(ir,jf),jf=1,n(2)+2),ir=1,irecnn)
       print *,'irecf',irecf,'mfak',mfak
       print *,'iv',((iv(ir,jf),jf=1,n(2)+2),ir=1,3)
...
6      format(40i4,3i8,20i4,20i4,20i4,20i4,2i5)       
       
       
       open (unit=3,file='gausf2',access='sequential')
...
!       'formatted',recl=nzz2,status='old')
       ...
       read (2,2,rec=irecf)((iv(ir,jf),jf=1,n(2)+2),ir=1,irecnn)
       print *,'irecf',irecf,'mfak',mfak,'n2',n(2),'irecnn',irecnn
!       print *,'iv',((iv(ir,jf),jf=1,n(2)+2),ir=1,20)
       do i=1,irecnn
!       print *,'irecf',irecf,'i',i,'iv',(iv(i,jf),jf=1,iv(i,2)+2)
       do jf=1,ismallm(kf,2)+2
       marr(jf)=ismallm(kf,jf)
       end do
...
!       print *,'ok2'
       ...
       do jf=1,ibigm(kf,2)+2
       mbarr(jf)=ibigm(kf,jf)
       end do
       call menmul
!       print *,'ok3'
       do jf=1,mcarr(2)+2
...
!       print *,'mminv',(mminv(kf,jf),jf=1,mminv(kf,2)+2)
       do jf=1,mminv(kf,2)+2
       mbarr(jf)=mminv(kf,jf)
       end do
       call menmul
       do jf=1,mcarr(2)+2
       karr(jf)=mcarr(jf)
       end do
!       print *,'ok4','itotarr',(itotarr(i,jf),jf=1,itotarr(i,2)+2)

       do jf=1,itotarr(i,2)+2
       kbarr(jf)=itotarr(i,jf)
       end do
       call mpadd(0)
!       print *,'ok5'
       do jf=1,kcarr(2)+2
...
!       print *,'ok6'
       ...
       kkll2=93+12*kkll
       open (unit=4,file='gausf4',access='direct',form=&
       'formatted',recl=kkll2,status='old')
       do i=1,irecnn
       read (4,499,rec=i)kia,kib,isgn,(littr(jf),jf=1,20),((nar(kk,jf),jf=&
       1,3),kk=1,kkll)
       
       do jf=1,3
       nararr(i,jf)=nar(kmx1+iig,jf)
       
       end do
       if (nararr(i,3).lt.20)goto 600
       do jf=1,ipdf(5,2)+2
       karr(jf)=ipdf(5,jf)
       end do
       do jf=1,nararr(i,2)+2
       kbarr(jf)=nararr(i,jf)
       end do
       call mpadd(1)
       do jf=1,kcarr(2)+2
       kbarr(jf)=kcarr(jf)
...
600    end do
       ...
       do kkk=1,5
       imsw=1
       read (2,2,rec=kkk)((iv(ir,jf),jf=1,n(2)+2),ir=1,irecnn)
       print *,'kkk',kkk
!       print *,'iv',((iv(ir,jf),jf=1,n(2)+2),ir=1,20)
       do jf=1,ismallm(1,2)+2
       marr(jf)=ismallm(1,jf)
       end do
       do jf=1,ipdf(1,2)+2
       mbarr(jf)=ipdf(1,jf)
       end do
       ...
       print *,'pow-1',(mbarr(jf),jf=1,mbarr(2)+2)
...
! we now have combined vector in ivad
       
!  put power of base into ksum
       ksum(1)=0
       ksum(2)=0
...
612    do jf=1,ksum(2)+2
       ...
       call subgcd
       if ((kard(2).eq.1).and.(kard(3).eq.1))goto 613
       goto 615
613    do jf=1,ksum(2)+2
       karb(jf)=ksum(jf)
       end do
       call subgcd
       do jf=1,kard(2)+2
...
618    do jf=1,mtotl(2)+2
       ...
       print *,'mexp',(mexp(jf),jf=1,mexp(2)+2)
       goto 619


!  these instructions only for test
...
499   format (i6,i6,i1,20i4,5000i4)
...
202    do jf=1,iee(2)+2
       ...
       print *,'jbase',(jbase(jf),jf=1,jbase(2)+2)
...
300    print *,'base=',(jbase(jf),jf=1,jbase(2)+2)
       print *,'vector 1',nnj,'vector 2',kkk
       print *,'check target',(igg2(jf),jf=1,igg2(2)+2)
       if (imsw.ne.3)goto 620
       print *,'exponent',(mexp(jf),jf=1,mexp(2)+2)
       print *,'prime',(ibpr(jf),jf=1,ibpr(2)+2)
       stop

620    do jf=1,igg2(2)+2
       ipretx(jf)=igg2(jf)
       end do
       do jf=1,lsum(2)+2
       mexp(jf)=lsum(jf)
...
!      now check that powering base by exponent gives target
       ...
       iee(3)=1
410    do jf=2,iee(2)+2
       if (iee(jf).lt.mexp(jf))goto 401
       if (iee(jf).gt.mexp(jf))goto 402
       end do
...
450    if ((iee(2).eq.1).and.(iee(3).eq.1))goto 500
       do jf=1,iee(2)+2
       marr(jf)=iee(jf)
       end do
       mbarr(1)=0
...
!       print *,'iee',(iee(jf),jf=1,iee(2)+2)
       ...
       do jf=2,itex(2)+2
       if (itex(jf).lt.iee(jf))goto 450
       if (itex(jf).gt.iee(jf))goto 460
       end do
460    do jf=1,itex(2)+2
       karr(jf)=itex(jf)
       end do
       do jf=1,iee(2)+2
       kbarr(jf)=iee(jf)
       ...
       goto 450
500    print *,'targ=',(jtarg(jf),jf=1,jtarg(2)+2)

       print *,'check target',(igg2(jf),jf=1,igg2(2)+2)
       print *,'ipretx',(ipretx(jf),jf=1,ipretx(2)+2)
       do jf=1,ipretx(2)+2
       marr(jf)=ipretx(jf)
       end do
       do jf=1,igg2(2)+2
       ...
       call mendiv
       print *,'product',(mcarr(jf),jf=1,mcarr(2)+2)
!       print *,'marr',(marr(jf),jf=1,marr(2)+2)
!       print *,'mbarr',(mbarr(jf),jf=1,mbarr(2)+2)
       print *,'nnj',nnj,'kkk',kkk
       if ((mcarr(2).eq.1).and.(mcarr(3).eq.1))goto 610
! should be goto 611
...
610    if (imsw.eq.1)goto 617
       imsw=3
       goto 618
617    imsw=2
       goto 612
611    if (imsw.eq.1)goto 615
       do jf=1,igcd2(2)+2
       marr(jf)=igcd2(jf)
       end do
