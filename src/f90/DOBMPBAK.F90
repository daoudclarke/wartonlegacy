      ...
      dimension kndx(4000),kr(2000)
      real krecarr(4000),lgpr(400),lglm
      ktim=1
      ...
      irecnn=0
      kkmx=0
      open(unit=1,file='sieve',access='direct',form=&
      'formatted',recl=40000,status='old')
...
300   ib=ib
      kia=ia
      kib=ib
      call sieve(kia,kib,irecnN,kkmx,1)
      if (norma(2).lt.3)goto 3002
      numb=norma(3)*10000+norma(4)
      rel=numb
      krecarr(ia*2-1)=log10(rel)+4*norma(2)-8
      kndx(ia*2-1)=1
      print *,'norma',(norma(jf),jf=1,norma(2)+2)
      print *,'log',krecarr(ia*2-1)

3007  kia=ia *(-1)
      call sieve(kia,kib,irecnn,kkmx,1)
      if (norma(2).lt.3)goto 3003
      numb=norma(3)*10000+norma(4)
      rel=numb
      krecarr(ia*2)=log10(rel)+4*norma(2)-8
      kndx(ia*2)=1
3004  ia=ia+1
...
340   ib=ib+1
      if(ib.gt.isivl)goto 400
      ia=1
3404  write(1,3401,rec=ib-1)(kndx(jf),krecarr(jf),jf=1,4000)      
      if (ib.ne.6)goto 3402
      print *,(kndx(jf),jf=1,200)
      
3401  format(4000(i1,f9.6))      
3402  do jf=1,4000
      krecarr(jf)=0
      kndx(jf)=0
      end do
...
400   ia=1
      ...
      read(1,3401,rec=jz)(kndx(jf),krecarr(jf),jf=1,4000)
      if (jz.ne.5)goto 4001
      
4001  do kz=1,100
      
      if (kndx(kz*2-1).eq.0)goto 500
      kia=kz
      kib=jz
...
5003  call mpdiv(ilen,ilen2,irlen,icont,iswq)
      ...
      if (ipr(iz).lt.10000)goto 5112
      kara(3)=int(ipr(iz)/10000)
...
5112  kara(3)=ipr(iz)
      kara(1)=0
      kara(2)=1
5114  if (kib.lt.10000)goto 5116
...
5120  karr(1)=abs(kia)
      ilen=1
5122  call mpmul(ilen,ilen2,ilen3)
      do i=1,ilen3
      karr(i)=kcarr(i)
      end do
      ilen=ilen3
      ...
      ilen2=kara(2)
      call mpdiv(ilen,ilen2,irlen,icont,iswq)
      if (irlen.eq.0)goto 5126
      if (kia.gt.0)goto 5124
      kbarr(2)=irlen
      do i=1,irlen
      kbarr(i+2)=irrr(i)
      end do
      ...
      call mpadd(1)
!     nnb for small examples residue will be less than 10000
      kr(iz)=kcarr(3)
      print *,'iz',iz,'kr',kr(iz),kia,kib
      ick=jz
      goto 511
5124  kr(iz)=irrr(1)
      print *,'iz',iz,'kr',kr(iz),kia,kib
...
5126  kr(iz)=0
      ...
      ick=jz
      goto 511
500   if (kndx(kz*2).eq.0)goto 510
      kia=kz*(-1)
      kib=jz
      if (iswiz.eq.0)goto 5130
      goto 510
5130  iswiz=1
      goto 5005
510   end do
...
511   end do
      ...
      do jz=1,100
      kib=jz
      read(1,3401,rec=kib)(kndx(jf),krecarr(jf),jf=1,4000)
      
      do iz=2,205
      if (kr(iz).eq.99999)goto 615
      karr(1)=kib
      ilen=1
      kbarr(1)=kr(iz)
      ilen2=1
...
604   markm=markb
      markbn=ipr(iz)-markb
6041  if ((markb.gt.2000).and.(markbn.gt.2000))goto 605
      if ((markb.gt.2000).or.(markb.eq.0))goto 606
      if (kndx(markb*2-1).ne.1)goto 606
      krecarr(markb*2-1)=krecarr(markb*2-1)-lgpr(iz)
606   if ((markbn.gt.2000).or.(markbn.eq.0))goto 6062
...
6051  if ((markl.lt.1).and.(markln.lt.1))goto 615
      if ((markl.lt.1).or.(markl.gt.2000))goto 616
      if (kndx(markl*2-1).ne.1)goto 616
      krecarr(markl*2-1)=krecarr(markl*2-1)-lgpr(iz)
616   if ((markln.gt.2000).or.(markln.lt.1))goto 6162

      if (kndx(markl*2).ne.1)goto 6162
      krecarr(markln*2)=krecarr(markln*2)-lgpr(iz)
6162  markl=markl-ipr(iz)
      markln=markln-ipr(iz)
      goto 6051
615   end do
      write(1,3401,rec=kib)(kndx(jf),krecarr(jf),jf=1,4000)
      end do
      ...
      read(1,3401,rec=jz)(kndx(jf),krecarr(jf),jf=1,4000)
      do kz=1,4000
      if ((kndx(kz).ne.2).and.(kndx(kz).ne.1))goto 710
      isgn=mod(kz,2)
      kia=int(kz/2)+1
      if (isgn.eq.0)goto 699
      kia =kia*(-1)
699   kib=jz         
      if (kndx(kz).eq.2)goto 702
      if (krecarr(kz).gt.lglm)goto 710
702   kpoin=0
