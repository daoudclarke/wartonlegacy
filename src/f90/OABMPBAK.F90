      ...
      program oabmpbak
!     lattice sieve
!     sieving for large numbers,single & double GNFS sieves     
      ...
      common kr(10001,10),kndx(8000),krecarr(8000),zlog(10000),ipol1(6,50)
      dimension iprar(4),invarr(10000)
      dimension littr(20),iabp(30),iabpn(30),ity(30),normar(20)
      ...
      'formatted',recl=nnrec,status='old')
      do iz2=1,9
      read (1,12347,rec=iz2)(kr(iz,iz2),iz=1,irecll)
      end do
      do iz=1,10
      print *,(kr(iz,iz2),iz2=1,10)
      end do
      
      iaaa=40
      ibbb=84
      call subgcd(iaaa,ibbb,igcd2)
      print *,'igcd2',igcd2
      
      
      
      nnrec7=5*irecll
      open (unit=7,file='invers',access='direct',form=&
      'formatted',recl=nnrec7,status='old')
      ...
      open(unit=4,file='lits',access='sequential')
!      goto 114
      jconll=0
113   read (4,*)irecnn,kia,kib,icur,(littr(j1),j1=1,20),(normar(j2)&
      ,j2=1,20)
      read (4,*)irecnn,(iabp(i),iabpn(i),ity(i),i=1,icur)
      print *,'littr',(littr(j1),j1=1,20),'kia',kia,'kib',kib
      print *,'normar',(normar(jf),jf=1,20)
      do i=1,icur
      print *,'i',i,'iabp',iabp(i),'iabpn',iabpn(i),'ity',ity(i) 
      end do
      jconll=jconll+1
      if (jconll.ne.3)goto 113
      stop
114   a=a      
      n=1
...
!      goto 8
      ...
      do j=1,3100
      if (i.eq.j)goto 9
      ihhb=mod(ipr(i),ipr(j))
      call subbw6(ipr(j),ihhb,ihhv)
      invarr(j)=ihhv
9     end do
      print *,'before i',i
!      write(7,7,rec=i-49)(invarr(jf),jf=1,4500)
      do j=3101,6000
      if (ipr(i).lt.10000)goto 94
      karr(1)=ipr(i)/10000
      karr(2)=ipr(i)-karr(1)*10000
      ilen=2
      goto 95
94    karr(1)=ipr(i)
      ilen=1
95    kbarr(1)=ipr(j)/10000
      kbarr(2)=ipr(j)-kbarr(1)*10000
      ilen2=2
      kara(3)=kbarr(1)
      kara(4)=kbarr(2)
      kara(1)=0
      ...
      karb(2)=irlen
      call mpgcd
      if (karv(2).eq.1)goto 91
      invarr(j)=karv(3)*10000+karv(4)
      goto 92
91    invarr(j)=karv(3)
92    end do
      write(7,7,rec=i-49)(invarr(jf),jf=1,irecll)
      print *,'record i',i
...
!      read (2)kr
      ...
      icc=0
      do iz=1,irecll
      if (kr(iz,5).eq.999999)goto 12378
      icc=icc+1
      print *,'iz',iz,'icc',icc
12378 end do      
      print *,'icc',icc
      ...
      icc=0
      do iz=1,irecll
      if ((kr(iz,1).ne.999999).and.(kr(iz,2).eq.999999))goto 12346
      goto 12377
12346 icc=icc+1
...
5005  call sieve(kia,kib,irecnn,kkmx,1)
      do jf=3,norma(2)+2
      karr(jf-2)=norma(jf)
      end do
      ilen=norma(2)
...
!     'formatted',recl=27000,status='old')
      do iz2=1,10
      write(1,12347,rec=iz2)(kr(iz,iz2),iz=1,4500)
      end do
12347 format(10000i6)
      close (unit=1)
!     change parameter
12349 do i=2,irecll
      rel=ipr(i)
...
!      call fscomp(1320,vlgls,313,295,kspec,vkazer,3)
      
      do kib=1,isivl
      
      do ibz=150,50,-1
      if (kr(ibz,1).eq.999999)goto 20007
      read (7,7,rec=ibz-49)(invarr(jf),jf=1,3100)
!      print *,'kribz',ibz,(kr(ibz,jf),jf=1,10)
      do ibz2=10,1,-1
      if (kr(ibz,ibz2).eq.999999)goto 20001
      ispecp=ipr(ibz)
      itt=kr(ibz,ibz2)
      
!      print *,'kib',kib,'itt',itt,'ispecp',ispecp
      ispeca=mod(kib*itt,ispecp)
!      print *,'ispeca',ispeca
      call fscomp(kib,vlgls,ispecp,ispeca,kspec,vkazer,3)
      
      print *,'kib',kib,'ibz',ibz,'irecnn',irecnn
      
      
!      print *,'ibz',ibz,'ibz2',ibz2,'kr',kr(ibz,ibz2)
      
      if (ibz.eq.150)goto 20003
...
!      print *,'jbz',jbz
      ...
      jspecp=ipr(jbz)
      jtt=kr(jbz,jbz2)
      jspeca=mod(kib*jtt,jspecp)
!      ihha=jbz
!      ihhb=mod(ispecp,jspecp)
!      print *,'ihs1',ihha,ihhb
!      call subbw6(ihha,ihhb,ihhv)
!      read (7,7,rec=ihha)(invarr(jf),jf=1,4500)
      ihhv=invarr(jbz)
      markb=(jspeca-ispeca)*ihhv
      
      markb=mod(markb,jspecp)
      
      if (markb.ge.0)goto 12350
      markb=markb+jspecp
      
12350 iz=jbz
!      print *,'markb',markb
      if (markb.ne.0)goto 2603
      kspec=0

2603  markbn=ipr(iz)-markb
2604  markm=markb
      markbn=ipr(iz)-markb
26041 if ((markb.gt.1000).and.(markbn.gt.1000))goto 2605
      if ((markb.gt.1000).or.(markb.eq.0))goto 2606
      if (kndx(markb*2-1).ne.1)goto 2606
      kndx(markb*2-1)=0
2606  if ((markbn.gt.1000).or.(markbn.eq.0))goto 26062
      if (kndx(markbn*2).ne.1)goto 26062
      kndx(markbn*2)=0
26062  markb=markb+ipr(iz)
      markbn=markbn+ipr(iz)
      goto 26041
2605  markl=markm-ipr(iz)
      markln=ipr(iz)-markl
26051 if ((markl.lt.1).and.(markln.lt.1))goto 2615
      if ((markl.lt.1).or.(markl.gt.1000))goto 2616
      if (kndx(markl*2-1).ne.1)goto 2616
      kndx(markl*2-1)=0
2616   if ((markln.gt.1000).or.(markln.lt.1))goto 26162

      if (kndx(markln*2).ne.1)goto 26162
      kndx(markln*2)=0
26162 markl=markl-ipr(iz)
...
!     end of checking whether lattice point can be used
...
20003 a=a       
      ...
      iconf=0
      do jbz=2,irecll
      if ((jbz.ge.ispecp).and.(jbz.lt.151))goto 20101
      if (jbz.lt.3101)goto 20004
!      iprar(1)=0
!      iprar(2)=2
!      iprar(3)=ipr(jbz)/10000
!      iprar(4)=ipr(jbz)-iprar(3)*10000
20004 do jbz2=1,10
      
      if (kr(jbz,jbz2).eq.999999)goto 3615
      if (jbz.lt.3101)goto 20050
      itt=mod(ispecp,ipr(jbz))
      jspecp=ipr(jbz)
      goto 93
      if (itt.lt.10000)goto 20010
      karb(1)=0
      karb(2)=2
      karb(3)=itt/10000
      karb(4)=itt-karb(3)*10000
      goto 20011
20010 karb(1)=0
      karb(2)=1
      karb(3)=itt
20011 kara(1)=0
      ...
      kara(4)=iprar(4)
      call mpgcd
93    jspeca=kib*kr(jbz,jbz2)
      jspeca=mod(jspeca,jspecp)
      jtt=mod(jspeca-ispeca,jspecp)
!      if (karv(2).eq.1)goto 20012
!      ihhv=karv(3)*10000+karv(4)
      ihhv=invarr(jbz)
      goto 20013
20012 ihhv=karv(3)
20013 markb=mod(ihhv*jtt,jspecp)
      iz=jbz
      if (markb.ge.0)goto 20014
      markb=markb+jspecp
20014 goto 20051


20050 ispeca=mod(kib*itt,ispecp)
      jspecp=ipr(jbz)
      jtt=kr(jbz,jbz2)
      jspeca=mod(kib*jtt,jspecp)
      ihha=jspecp
...
!      read (7,7,rec=ihha)(invarr(jf),jf=1,4500)
      ...
      markb=mod(markb,jspecp)
      iz=jbz
      if (markb.ge.0)goto 20051
      markb=markb+jspecp
20051 if (markb.eq.0)goto 3602
      goto 3603
3602  if (kspec.eq.0)goto 3604
      vkazer=vkazer-lgpr(iz)
      goto 3604

3603  markbn=ipr(iz)-markb
3604  markm=markb
      markbn=ipr(iz)-markb
36041 if ((markb.gt.1000).and.(markbn.gt.1000))goto 3605
      if ((markb.gt.1000).or.(markb.eq.0))goto 3606
      if (kndx(markb*2-1).ne.1)goto 3606
      krecarr(markb*2-1)=krecarr(markb*2-1)-lgpr(iz)
!      print *,'krecarr1',krecarr(markb*2-1)
3606  if ((markbn.gt.1000).or.(markbn.eq.0))goto 36062
      if (kndx(markbn*2).ne.1)goto 36062
      krecarr(markbn*2)=krecarr(markbn*2)-lgpr(iz)
!      print *,'krecarr2',krecarr(markbn*2)
36062  markb=markb+ipr(iz)
      markbn=markbn+ipr(iz)
      goto 36041
3605  markl=markm-ipr(iz)
      markln=ipr(iz)-markl
36051 if ((markl.lt.1).and.(markln.lt.1))goto 3615
      if ((markl.lt.1).or.(markl.gt.1000))goto 3616
      if (kndx(markl*2-1).ne.1)goto 3616
      krecarr(markl*2-1)=krecarr(markl*2-1)-lgpr(iz)
!      print *,'krecarr3',krecarr(markl*2-1)
3616   if ((markln.gt.1000).or.(markln.lt.1))goto 36162

      if (kndx(markln*2).ne.1)goto 36162
      krecarr(markln*2)=krecarr(markln*2)-lgpr(iz)
!      print *,'krecarr4',krecarr(markln*2)
36162 markl=markl-ipr(iz)
      markln=markln-ipr(iz)
      goto 36051
3615  end do
20101 end do
      if (kspec.eq.0)goto 20200
      if (vkazer.gt.vlgls)goto 20200
      kia=ispeca
      call sieve(kia,kib,irecnn,kkmx,0)
20200 do i=1,2000
      if (kndx(i).eq.0)goto 20300
      if (krecarr(i).gt.vlgls)goto 20300
      
      isgn=mod(i,2)
      kmul=i/2
      if (isgn.eq.1)goto 20299
      kmul=kmul*(-1)
      goto 20302
20299 kmul=kmul+1
20302 kpoin=0
      kia=ispeca+kmul*ispecp
      call sieve(kia,kib,irecnn,kkmx,kpoin)
20300 end do
20001 end do
20007 end do
      end do
...
3401  format(4000(i1,f9.5))      
      goto 16159
      call fscomp(kib,vlgls,ispecp,ispeca,kspec,vkazer,1)
...
1604  markm=markb
      markbn=ipr(iz)-markb
16041 if ((markb.gt.40000).and.(markbn.gt.40000))goto 1615
      if ((markb.gt.40000).or.(markb.eq.0))goto 1606
      if (kndx(markb*2-1).ne.1)goto 1606
      krecarr(markb*2-1)=krecarr(markb*2-1)-lgpr(iz)
1606  if ((markbn.gt.40000).or.(markbn.eq.0))goto 16062
...
1615  end do
16159 call fscomp(kib,vlgls,ispecp,ispeca,kspec,vkazer,2)
...
604   markm=markb
      markbn=ipr(iz)-markb
6041  if ((markb.gt.40000).and.(markbn.gt.40000))goto 615
      if ((markb.gt.40000).or.(markb.eq.0))goto 606
      if (kndx(markb*2-1).ne.1)goto 606
      krecarr(markb*2-1)=krecarr(markb*2-1)-lgpr(iz)
606   if ((markbn.gt.40000).or.(markbn.eq.0))goto 6062
...
420   print *,'max no of primes=',kkmx
      ...
      subroutine sieve(kia,kib,irecnn,kkmx,kpoin)
      common karr(50),kbarr(50),kcarr(200),ipqt(50),irrr(50)
      common mnum(50)
      common ia5(5),ia4(5),ia3(5),ia2(5),ia1(5),ia0(5),m1(5),m2(5)
      common ipr(65000),norma(20)
...
1306  iab=iab+1
      iprx1= ipr(iab)
      if (kr(iab,1).eq.999999)goto 1306
      iarp(1)=int(iprx1/10000)
      iarp(2)=iprx1-iarp(1)*10000
...
1500  return
      ...
      subroutine fscomp(kib,vlgls,ispecp,ispeca,kspec,vkazer,kjk)
      common karr(50),kbarr(50),kcarr(200),ipqt(50),irrr(50)
      common mnum(50)
      common ia5(5),ia4(5),ia3(5),ia2(5),ia1(5),ia0(5),m1(5),m2(5)
      common ipr(65000),norma(20)
      ...
      dimension ipowr(5,6,50),itempb(20),isspar(5,20),iapar(5,20)
      real krecarr
      if (kjk.eq.2)goto 60
...
60    do i=1,5
      ...
      ipowr(5,6,3)=1
      if (kjk.eq.2)goto 49
      if (ispecp.lt.10000)goto 70
      isspar(1,1)=0
      isspar(1,2)=2
      isspar(1,3)=ispecp/10000
      isspar(1,4)=ispecp-isspar(1,3)*10000
      goto 71
70    isspar(1,1)=0
      isspar(1,2)=1
      isspar(1,3)=ispecp
71    do jf=3,isspar(1,2)+2
      karr(jf-2)=isspar(1,jf)
      kbarr(jf-2)=isspar(1,jf)
      end do
      ilen=isspar(1,2)
      ilen2=isspar(1,2)
      do ii=2,5
      ...
      karr(jf-2)=ipowr(ii,jj,jf)
      end do
      ilen=ipowr(ii,jj,2)
      do jf=3,isspar(jj-ii,2)+2
      kbarr(jf-2)=isspar(jj-ii,jf)
      end do
      ilen2=isspar(jj-ii,2)
      call mpmul(ilen,ilen2,ilen3)
      do jf=1,ilen3
      ipowr(ii,jj,jf+2)=kcarr(jf)
      end do
      ipowr(ii,jj,2)=ilen3
!      print *,'ipowr','iijj',ii,jj,(ipowr(ii,jj,jf),jf=1,ipowr(ii,jj,2)+2)
      
      end do
      
      end do
      
      if (ispeca.lt.10000)goto 80
      iapar(1,1)=0
      iapar(1,2)=2
      iapar(1,3)=ispeca/10000
      iapar(1,4)=ispeca-iapar(1,3)*10000
      goto 81
80    iapar(1,1)=0      
      iapar(1,2)=1
      iapar(1,3)=ispeca
81    do jf=3,iapar(1,2)+2      
      karr(jf-2)=iapar(1,jf)
      kbarr(jf-2)=iapar(1,jf)
      end do
      ilen=iapar(1,2)
      ilen2=iapar(1,2)
      do ii=2,5
      ...
      do jf=1,ilen3
      kbarr(jf)=kcarr(jf)
      iapar(ii,jf+2)=kcarr(jf)
      end do
      iapar(ii,1)=0
      iapar(ii,2)=ilen3
      ilen2=ilen3
!      print *,'ispeca',ispeca,'ii',ii,(iapar(ii,jf),jf=1,iapar(ii,2)+2)
      end do
      ...
      little=kib
      ibbb=ispeca
      call subgcd(ibbb,little,igcd2)
      if (igcd2.ne.1)goto 110
      kspec=1
      goto 112
110   kspec=0
112   do i=1,2000
...
114   end do
      ...
      ibig=iccc
      call subgcd(ibig,little,igcd2)
      if (igcd2.ne.1)goto 116
      kndx(i*2)=1
116   end do
!      print *,'ispeca',ispeca,'ispecp',ispecp,'kib',kib
!     print *,'kndx',(kndx(jf),jf=1,100)
      
      
      if (kspec.eq.0)goto 49
      call sieve(ispeca,kib,irecnn,kkmx,1)
      rel=zlog(norma(3))+4*norma(2)-4-vsub
      vkazer=rel
...
49    do ibig=1,2
      ...
      call sieve(1,kib,irecnn,kkmx,1)
...
521   if (kjk.eq.2)goto 52
      do jf=1,ipol1(6,2)+2
      ixold(6,jf)=ipol1(6,jf)
      end do
      do ii=1,5
      if (ipol1(ii,2).eq.0)goto 88
      do jf=3,ipol1(ii,2)+2
      karr(jf-2)=ipol1(ii,jf)
      end do
      ilen=ipol1(ii,2)
      do jf=3,iapar(6-ii,2)+2
      kbarr(jf-2)=iapar(6-ii,jf)
      end do
      ilen2=iapar(6-ii,2)
      call mpmul(ilen,ilen2,ilen3)
      do jf=1,ilen3
...
88    end do
!      print *,'ispeca',ispeca,(ixold(6,jf),jf=1,ixold(6,2)+2)
...
!     print *,'iipol21',(ipol2(ii,jf),jf=1,ipol2(ii,2)+2),'ii',ii,'ibig',ibig
    ...
    if (kjk.eq.2)goto 90
      ...
      karr(2)=ilen3
      karr(1)=ipol1(ii,1)
      do jf=1,ixold(6-kk1,2)+2
      kbarr(jf)=ixold(6-kk1,jf)
      end do
      call mpadd(0)
      do jf=1,kcarr(2)+2
      ixold(6-kk1,jf)=kcarr(jf)
      end do
91    end do     
      goto 92
...
!     if (krecarr(iconz).gt.vlgls)goto 19
     if (ixnew(6,2).eq.0)goto 17
     rel=zlog(ixnew(6,3))+4*ixnew(6,2)-4
!     print *,'ixnew6',(ixnew(6,jf),jf=1,ixnew(6,2)+2)
!     print *,'rel',rel
     
     if (jz.ne.2)goto 141
     
141  krecarr(iconz)=rel
     if (kjk.eq.2)goto 14
     krecarr(iconz)=krecarr(iconz)-vsub
!     print *,'vsub',vsub,'krecarr',krecarr(iconz)
...
100  return
     ...
     subroutine subbw6(ihha,ihhb,ihhv)
     ia=ihha
     ib=ihhb
     ib=mod(ib,ia)
     if (ib.ge.0)goto 10
...
830  iv=(id-ia*iu)/ib
     ...
     if (iv.ge.0)goto 870
     iv=iv+ia
870  if (id.gt.1)goto 890
!     print *,'inverse=',iv,'ia=',ia,'ib=',ib
     goto 890
888  iv=ib
890  ihhv=iv
     return
