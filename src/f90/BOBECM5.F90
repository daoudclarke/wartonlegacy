...
!     fast multiplication version (once)     
      ...
      dimension ixdf(40,30,50),iydf(40,30,50)
      dimension kdoub2(201,300)
      dimension ixdf2(40,30,50),iydf2(40,30,50),iqarr(300),ie(300)
      dimension ixpow(40,30,50),iypow(40,30,50)
      iaa=31
...
40    a=a      
      ...
      common ix1(30,50),iy1(30,50),ix2(30,50),iy2(30,50),icc(30,50)
      common n(50)
      dimension karrp(50),kbarrp(50),karra(4,50),kbarra(4,50)
      dimension ilena(4),ilena2(4),joina(2,50),joinb(2,50),ibdf(50)
      dimension iarr(50),ibrr(50),iadf(50)
      ilenp=ilen
      ilenp2=ilen2
      do i=1,ilen
      karrp(i)=karr(i)
...
2     kkl=1
      do i=1,4
      if (ilenp.lt.(4-i)*ilenas+1)goto 210
      ilena(i)=ilenp-(4-i)*ilenas
      ...
      if (i.eq.4)goto 212
      do i1=i+1,4
      ired=ilena(i)+ilenas*(kkl-2)+1 
      ijk=1
      do jf=ired,ired+ilenas-1
      if (karr(jf).eq.0)goto 204
      ilena(i1)=ilenas+1-ijk
      do jk=1,ilena(i1)
      karra(i1,jk)=karr(jf-1+jk)
      end do
      goto 203
204   ijk=ijk+1
      end do
...
210   end do
      print *,'mult error'
      stop
212   print *,'ilenas',ilenas
      print *,ilena(1),ilena(2),ilena(3),ilena(4)
      do i=1,4
...
213   end do
      kkl=1
      do i=1,4
      if (ilenp2.lt.(4-i)*ilenas+1)goto 310
      ilena2(i)=ilenp2-(4-i)*ilenas
      do jf=1,ilena2(i)
      kbarra(i,jf)=kbarr(jf)
      end do
      kkl=2
      if (i.eq.4)goto 312
      do i1=i+1,4
      ired=ilena2(i)+ilenas*(kkl-2)+1
      ijk=1
      do jf=1,ired+ilenas-1
      if (kbarr(jf).eq.0)goto 304
      ilena2(i1)=ilenas+1-ijk
      do jk=1,ilena2(i1)
      kbarra(i1,jk)=kbarr(jk-1+jk)
      end do
      goto 303
304   ijk=ijk+1 
      end do
...
310   end do
      print 'mult error 2'
      stop
312   print *,ilena2(1),ilena2(2),ilena2(3),ilena2(4)
      do i=1,4
      if (ilena2(i).eq.0)goto 313
      print *,'i',i,'kbarra',(kbarra(i,jf),jf=1,ilena2(i))
313   end do
!     form concatenations      
      if (ilena(1).eq.0)goto 400
      do jf=1,ilena(1)+ilenas
      joina(1,jf+2)=karrp(jf)
      end do
      joina(1,1)=0
      joina(1,2)=ilena(1)+ilenas
      goto 410
400   if (ilena(2).eq.0)goto 402      
      do jf=1,ilena(2)
      joina(1,jf+2)=karrp(jf)
      end do
402   joina(1,1)=0
      joina(1,2)=ilena(2)
410   if (ilena(3).eq.0)goto 416      
      do jf=joina(1,2)+1,joina(1,2)+ilenas+ilenas   
      if (karrp(jf).eq.0)goto 412
      goto 414
412   end do
      joina(2,1)=0
      joina(2,2)=0
      goto 420
414   ilent=joina(1,2)+ilenas+ilenas
      inj=3
      do jk=jf,ilent
      joina(2,inj)=karrp(jk)
      inj=inj+1
      end do
      joina(2,1)=0
      joina(2,2)=ilent+1-jf
      goto 420
416   do jf=1,ilena(4)
      joina(1,jf+2)=karrp(jf)
      end do
...
420   print *,'joins',(joina(1,jf),jf=1,joina(1,2) +2)
      print *,'joins',(joina(2,jf),jf=1,joina(2,2)+2)
      if (ilena2(1).eq.0)goto 500
      do jf=1,ilena2(1)+ilenas
      joinb(i,jf+2)=kbarrp(jf)
      end do
      joinb(1,1)=0
      joinb(1,2)=ilena2(1)+ilenas
      goto 510
500   if (ilena2(2).eq.0)goto 502
      do jf=1,ilena2(2)
      joinb(1,jf+2)=kbarrp(jf)
      end do
502   joinb(1,1)=0
      joinb(1,2)=ilena2(2)
510   if (ilena2(3).eq.0)goto 516
      do jf=joinb(1,2)+1,joinb(1,2)+ilenas+ilenas
      if (kbarrp(jf).eq.0)goto 512
      goto 514
512   end do
      joinb(2,1)=0
      joinb(2,2)=0
      goto 520
514   ilent=joinb(1,2)+ilenas+ilenas
      inj=3
      do jk=jf,ilent
      joinb(2,inj)=kbarrp(jk)
      inj=inj+1
      end do
      joinb(2,1)=0
      joinb(2,2)=ilent+1-jf
      goto 520
516   do jf=1,ilena2(4)
      joinb(2,jf+2)=kbarrp(jf)
      end do
      joinb(2,1)=0
      joinb(2,2)=ilena2(4)


520   print *,'bjoins',(joinb(1,jf),jf=1,joinb(1,2)+2)      
      print *,'bjoins',(joinb(2,jf),jf=1,joinb(2,2)+2)
      do jf=1,joina(1,2)+2
      karr(jf)=joina(1,jf)
      end do
      do jf=1,joina(2,2)+2
      ...
      isub=1
      goto 10
530   ibdf(1)=mod(ibdf(1)+isgn,2)
      ibdf(2)=ilen3
      do jf=1,ilen3
      ibdf(jf+2)=kcarr(jf)
      end do
      print *,'ibdf',(ibdf(jf),jf=1,ibdf(2)+2)
      do jf=1,ilena(1)
      karr(jf)=karra(1,jf)
      end do
      ilen=ilena(1)
...
540   do jf=1,ilen3
      ...
      do jf=kbarr(2)+2,kbarr(2)+1+ilenas
      kbarr(jf)=0
      end do
      call mpadd(0)
      do jf=1,kcarr(2)+2
...
100   if (isub.eq.1)goto 530
      if (isub.eq.2)goto 540
