...
!      of small matrices. works right to left         
       ...
       dimension indic(4000),iv(4000)
       dimension ic(4000),jpiv(4000),kct(1000),izzy(10)
       ...
       dimension iqi(200,200),iqi2(200,200),jqi2(200,200),irno(200)
       dimension matl(200,200),matr(200,200),icol(200),irow(200)
       dimension ilc(200),jrow(200),jcol(200)
       iig=5
       ia=-43
       ib=11
...
!       (nar(nk1,j),j=1,kmx2),(izzy(j1),j1=1,10)
       ...
       nk1=nk1+1
       print *,'k',k,'iqi',(iqi(k,jf),jf=1,nn)
...
301    end do
       ...
       call subbw6(ia,ib,ivv,iu)
       do ii=i+1,mm
       matl(ii,j)=iqi(ii,j)*ivv
       matl(ii,j)=mod(matl(ii,j),ip)
       end do
       goto 303
300    end do
       print *,'input matrix all zero error'
       stop
303    ist=i+1
!       print *,'ir1',irow(1),'ic1',icol(1),'jr1',jrow(1),'jc1',jcol(1)
       
       
       do kg=1,mm
       print *,'kg l',(matl(kg,jf),jf=1,nn),'kg r',(matr(kg,jf),jf=1,nn)
       end do
       print *,'ilc',(ilc(jf),jf=1,nn),'check zz'
       ir=0
       icnt=1
       do iii=ist,mm
       print *,'here we go icnt',icnt
       do jj=1,nn
       if (ilc(jj).ne.0)goto 350
       matl(iii,jj)=1
       print *,'lpiv','iii',iii,'jj',jj ,'icnt',icnt
       ixx=jj
       goto 349
350    end do
       goto 320
349    do jj=1,nn   
       if (ic(jj).ne.0)goto 304
       isum=iqi(iii,jj)
       print *,'iii',iii,'jj',jj,'isum',isum
       jjs=jj
...
305    a=a
       ...
       isum=isum-matr(ind1,jjs)* matl(iii,ind2)
       isum=mod(isum,ip)
       if (isum.ge.0)goto 306
       isum=isum+ip
306    end do
       if (isum.ne.0)goto 307
304    end do
       ir=ir+1
       irno(ir)=iii
       goto 319
307    matr(ixx,jjs)=isum
       
       print *,'iii',iii,'jjs',jjs,'matriiijjs',matr(iii,jjs),'icnt',icnt
       ...
       ic(jjs)=iii
       indic(iii)=jjs
       if (iii.eq.mm)goto 330
! filling in one col of matl       
       do i3=iii+1,mm
       ...
       jnd3=icol(j3)
       ind3=jrow(j3)
       isum=isum-matl(i3,jnd3)*matr(ind3,jjs)
       isum=mod(isum,ip)
       if (isum.ge.0)goto 308
       isum=isum+ip
308    end do
       print *,'ixx',ixx ,'matl21',matl(2,1),'icnt',icnt,'mm',mm
       
       matl(i3,jjs)=isum*ivv
       matl(i3,jjs)=mod(matl(i3,jjs),ip)
       print *,'i3',i3,'jjs',jjs,'matli3jjs',matl(i3,jjs)
       
       end do
       print *,'i3',i3,'jjs',jjs,'matli3jjs',matl(3,jjs)
       
! now filling in one row of matr       
!       if (jjs.eq.nn)goto 319
       do j3=1,nn
       if (ic(j3).ne.0)goto 310
       isum=iqi(iii,j3)
       do i3=1,icnt-1
       jnd3=icol(i3)
       ind3=jrow(i3)
       isum=isum-matl(iii,jnd3)*matr(ind3,j3)
       isum=mod(isum,ip)
       if (isum.ge.0)goto 311
       isum=isum+ip
311    end do
       goto 351
310    end do       
       goto 319
351    matr(iii,j3)=isum
       print *,'sec matl21',matl(2,1),'icnt',icnt
       print *,'j3',j3,'iii',iii,matr(iii,j3)
       
       goto 319
       
320    ir=ir+1
       irno(ir)=iii
       print *,'vector found ir=',ir,'irno=',irno(ir)
319    end do
       end do
       print *,'job completed'
       do i=1,mm
       print *,'i l',(matl(i,jf),jf=1,nn),'i r',(matr(i,jf),jf=1,nn)
       end do
...
330    print *,'job over'
       ...
       print *,'irow',(irow(jf),jf=1,mm)
       print *,'icol',(icol(jf),jf=1,mm)
       print *,'jrow',(jrow(jf),jf=1,nn)
       print *,'jcol',(jcol(jf),jf=1,nn)
