     program bobmodj8 
! program for rigorous prime determination. Program bobmodj7     
! with 12 extra equations degree 1 and 2
! and 5 extra equations degree 4
...
!       common ncom(2,100),iprar(55),iansar(55),narc(55)
     ...
     common kpf(120),kqf(120),krf(120),iroota(2,120),khsq(120)
      common nfsol(5,100)
      dimension ireclen(100),irecarr(200),memfil1(12,30),memfil2(18,30)
      ...
      dimension memfil3(4,60),memfil4(21,100),iprar(100)
      ncom(1,1)=0
      ncom(1,2)=1
      ncom(1,3)=31
      ncom(2,1)=0
...
!      call cornsq
!      stop
      open (unit=4,file='modjpol4',access='sequential')
...
1006  format (65000i6)      
      ...
      print *,'ipr10835',ipr(10835),'ipr10836',ipr(10836)
...
!      print *,'memfil3 3',(memfil3(3,jf),jf=1,memfil3(3,4))
      open (unit=2,file='modjlen4',access='direct',form=&
      'formatted',recl=600,status='old')
      read (2,1005,rec=1)(ireclen(jf),jf=1,12)
      iccc=0
501   iccc=iccc+1
      limt=ireclen(iccc)
      
      read (4,*,end=500)(irecarr(jf),jf=1,limt) 
      if (irecarr(3).eq.1)goto 511
      icd2=icd2+1
      do jf=1,limt
      memfil2(icd2,jf)=irecarr(jf)
      end do
...
511   icd1=icd1+1
      ...
      print *,'dis1',(irecarr(jf),jf=1,limt)
      goto 501
500   print *,'icd1',icd1,'icd2',icd2
      close (unit=2)
      close (unit=4)
      open (unit=4,file='modjpol5',access='sequential')
      open (unit=2,file='modjlen5',access='direct',form=&
      'formatted',recl=600,status='old')
      read (2,1005,rec=1)(ireclen(jf),jf=1,8)
      iccc=0
      icd3=0
525   iccc=iccc+1
      limt=ireclen(iccc)
      
      read (4,*,end=526)(irecarr(jf),jf=1,limt)
      if (irecarr(3).eq.3)goto 527
      icd4=icd4+1
      do jf=1,limt
      memfil4(icd4,jf)=irecarr(jf)
      end do
      print *,'disc4',(irecarr(jf),jf=1,limt)
      goto 525
527   icd3=icd3+1
      do jf=1,limt
      memfil3(icd3,jf)=irecarr(jf)
      end do
      print *,'disc3',(irecarr(jf),jf=1,limt)
      goto 525
526   print *,'icd4',icd4,'icd3',icd3
      close (unit=2)
      close(unit=4)
      open (unit=4,file='modjpol6',access='sequential')
      open (unit=2,file='modjlen6',access='direct',form=&
      'formatted',recl=600,status='old')
      read (2,1005,rec=1)(ireclen(jf),jf=1,2)
      limt=ireclen(1)
      read (4,*)(irecarr(jf),jf=1,limt)
      do jf=1,limt
      memfil3(3,jf)=irecarr(jf)
      end do
      print *,'disc3',(irecarr(jf),jf=1,limt)
      limt=ireclen(2)
      read (4,*)(irecarr(jf),jf=1,limt)
      do jf=1,limt
      memfil3(4,jf)=irecarr(jf)
      end do
      print *,'disc3',(irecarr(jf),jf=1,limt)
      close (unit=2)
      close(unit=4)
      
!  manually input equation for disc-15 replace -60     
      memfil2(15,1)=9
      memfil2(15,2)=15
      memfil2(15,3)=2
      memfil2(15,4)=15
...
4     inach=1
! redundant equations      
      if (kdcorn(3).eq.12)goto 101
      if (kdcorn(3).eq.27)goto 101
      if (kdcorn(3).eq.28)goto 101
      if (kdcorn(3).eq.60)goto 101
      if (kdcorn(3).eq.64)goto 101
      if (kdcorn(3).eq.48)goto 101
      if (kdcorn(3).eq.75)goto 101
      if (kdcorn(3).eq.112)goto 101
      if (kdcorn(3).eq.72)goto 101
      if (kdcorn(3).eq.147)goto 101
      call nacchia(inach)
...
101   a=a
      ...
      memind1=memind1+1
      if (memind1.gt.12)goto 3
      memlis(nstcon)=memind1
      goto 1
3     memind2=memind2+1
      indstak(nstcon)=2
      if (memind2.gt.18)goto 5
...
59    print *,'failure to get started',' memind2=',memind2,'dis coun'&
      ,ijabcd
      stop
2     ijabcd=ijabcd+1
      ...
      if (memind1.le.12)goto 80
      if (memind2.gt.18)goto 310
...
70   nstcon=nstcon-1
     ...
     if ((indstak(nstcon).eq.1).and.(memlis(nstcon).eq.12))goto 73
...
306  if (memlis(nstcon).eq.4)goto 642     
...
73   memind1=13
     ...
     memlis(nstcon)=1
     goto 715
74   if (memlis(nstcon).eq.18)goto 308
     indstak(nstcon)=2
     memlis(nstcon)=memlis(nstcon)+1
...
! backtracking on 4th degee polynomial
     if (memlis(nstcon).eq.21)goto 70
     memlis(nstcon)=memlis(nstcon)+1
     indstak(nstcon)=4
...
700  if (idb.eq.20)goto 750      
     idb=idb+1
     goto 264 
660  if ((mdarr(2).eq.1).and.(mdarr(3).eq.1))goto 751

661  print *,(ip(jf),jf=1,ip(2)+2),'is composite'      
     indp=0
     goto 1000
751  print *,(ip(jf),jf=1,ip(2)+2),'is definitely prime'     
     indp=1
     goto 1000 
...
!      print *,'ntest',(ntest(jf),jf=1,ntest(2)+2)
      print *,'nn',(nn(jf),jf=1,nn(2)+2),'isq',isqurar(1,2,2),'nsq',nsq
...
318    print *,'ncom',(ncom(1,jk),jk=1,ncom(1,2)+2)
       call cornsq
       print *,'igarray22',igarray(2,2),'ipd2',ipd(2)
       do jf=1,igarray(2,2)+2
...
!       common n(100),jaa(100),jbb(100),kbcorn(200),kycorn(120)
       ...
       dimension ibbb(100),ittt(100),iyyy(100),ipow(100),ixxx(100)
       ibsw=0
       do jf=1,ipd(2)+2
       ip(jf)=ipd(jf)
       end do
...
!       print *,'iyyy',(iyyy(jf),jf=1,iyyy(2)+2),'nn',nn,'ipn',ipn(3)
...
!       print *,'secittt',(ittt(jf),jf=1,ittt(2)+2)
