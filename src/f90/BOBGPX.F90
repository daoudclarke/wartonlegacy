...
!     setting up sieving for ax+b      
      do jz=2,limprm
!      print *,'second setup jz',jz ,'limprm2',limprm2
!      if (gpr(jz).gt.90.0)goto 220
!      print *,'jz ok',jz,'ipr',ipr(jz)
      if (ipr(jz).lt.10000)goto 201
      iprar(1)=0
      iprar(2)=2
...
!      print *,'itemp1',(itemp1(jf),jf=1,itemp1(2)+2)
      do jf=3,ibeq(2)+2
      karr(jf-2)=ibeq(jf)
      end do
      ilen=ibeq(2)
...
203   itemp2(1)=0
      itemp2(2)=0
204   do jf=3,itemp1(2)+2
      karr(jf-2)=itemp1(jf)
      end do
      ilen=itemp1(2)
      if (itemp2(2).eq.0)goto 205
      do jf=3,itemp2(2)+2
      kbarr(jf-2)=itemp2(jf)
      end do
      ilen2=itemp2(2)
      call mpmul(ilen,ilen2,ilen3)
      do jf=1,ilen3
      ...
      mr(jz)=0
      do jf=irlen,1,-1
      mr(jz)=mr(jz)+irrr(jf)*10000**(irlen-jf)
      end do
      goto 200
205   mr(jz)=0
200   a=a
!      print *,'ipr',ipr(jz),'iaeq',(iaeq(jf),jf=1,iaeq(2)+2)
!      print *,'ibeq',(ibeq(jf),jf=1,ibeq(2)+2),'mr',mr(jz)
...
599   do jz=2,limprm2
      ...
      if (icdn.gt.-10000)goto 5991
      markl=mod(100000000+kr(jz,1),ipr(jz))
...
6101  if (ipr(jz).eq.2)goto 650
      ...
      if (icdn.gt.-10000)goto 6141
      markr=mod(100000000+kr(jz,2),ipr(jz))
...
650   end do
!     print *,'end of setup second phase'
      do jz=2,limprm
!      if (gpr(jz).gt.90.0)goto 210
!      print *,'jz second sieve',jz
      if (icdn.gt.-10000)goto 211
      mbarkl=mod(100000000+mr(jz),ipr(jz))
...
668   do kz=1,10000
      ...
      vimprm=mimprm
      vimprm3=vimprm+2.0
      if ((krecarr(kz).gt.vimprm).or.(mrecarr(jz).gt.vimprm3))goto 690
!     compute accurately      
...
!      print *,'rel3',rel3,'rel2',rel2,'lglm',lglm ,'mrec',mrecarr(kz)
      if ((rel2.gt.lglm).or.(rel3.gt.lglm*0.6))goto 690
...
720   print *,'maximum no of primes=',kkmx
      print *,'irecnn=',irecnn
      if (irecnn.eq.iprerec)goto 69001
      iprerec=irecnn
      iconq=iconq+1
...
!      if (irecnn.gt.100)goto 730
      if (irecnn.gt.limprm+limprm+iconq-1)goto 730
!       if (n(2).lt.4)goto 730
!       if (irecnn.gt.1000)goto 730
...
730   print *,'run completed',' no of records=',irecnn,'iconq',iconq
      ...
      (nbarr(1,jk),jk=1,50)
      open (unit=1,file='exppar',access='direct',form=&
      'formatted',recl=344,status='old')
1002  format (i6,i6,i8,i4,20i4,20i4,20i4,20i4)      
      write (1,1002,rec=1)irecnn,limprm,ijpow,nfak,(ibase(jf),jf=1,20),&
