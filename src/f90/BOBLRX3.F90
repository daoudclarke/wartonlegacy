       program boblrx3
!  program for reducing matrix prior to kernel computation
...
! first of 3 kernel-obtaining programs      
      ...
      common marr(200),mbarr(200),mcarr(200),mdarr(200)
       ...
       dimension indic(8000),iv(3000,20)
       dimension ic(8000),jpiv(8000),kct(1000)
       dimension nar(3000,20),littr(20),mpiv(3000,20),kpr(3000)
       dimension iharr(1000),larr(1000,20),n(60),jfak(30),jfreq(30)
       dimension ipd(50),mul(50),isum(20),irno(60),kset(1500)
...
1006   format (60i4)       
       ...
       open (unit=1,file='dsiev',access='direct',form=&
       'formatted',recl=kkll3,status='old')
       read (1,1004,rec=1)izzy1,kmx1,kmx2,izzy2,izzy3,(kpr(jf),jf=1,&
       kkll)
1004   format (5i5,3000i5)
!       close (unit=1)
!  split into kpr1 and kpr2
       print *,'kmx1',kmx1,'kmx2',kmx2 ,'kpr11',kpr(1),'kpr21',&
       kpr(kmx1+1),'kpr1e',kpr(kmx1),'kpr2e',kpr(kmx1+kmx2)
       ...
       icnt=0
       do i=kmx1+1,kmx1+kmx2
       do j=1,kmx1
       if (kpr(i).ne.kpr(j))goto 400
       icnt=icnt+1
...
401    end do
       print *,'icnt',icnt
       ired=icnt
       write (1,1004,rec=1)ired,kmx1,kmx2,izzy2,izzy3,(kpr(jf),jf=1,&
       kkll)
       close (unit=1)
       print *,'kset',(kset(jf),jf=1,kmx2)
       print *,'kpr1',(kpr(jf),jf=1,20)
       print *,'kpr2',(kpr(jf),jf=kmx1+1,kmx1+20)
...
6      format(i6,i6,i8,i4,20i4,20i4,20i4,20i4)       
       ...
       print *,'i','nar602',(nar(602,jf),jf=1,ipd(2)+2)
       end do
...
!       goto 60
       ...
       read (1,1,rec=i)kkb,ihitn,(littr(jf),jf=1,20),&
       ((nar(jk,jf),jf=1,ipd(2)+2),jk=1,kkll)
       do ijk=kmx1+1,kmx1+kmx2
       if (kset(ijk-kmx1).eq.0)goto 402
       ind2=kset(ijk-kmx1)
       ...
       if (karr(1).eq.0)goto 403
       do jf=1,ipd(2)+2
       kbarr(jf)=ipd(jf)
       end do
       call mpadd(0)
403    do jf=1,kcarr(2)+2
       nar(ind2,jf)=kcarr(jf)
       end do
       do jf=1,ipd(2)+2
       nar(ijk,jf)=0
       end do
...
!      'formatted',recl=nzz2,status='new')
      nzz3=318+5*nzz
1007  format (3i5,i3,60i5,2000i5)
...
!       print *,'ic',(ic(jf),jf=1,nn)
       ...
       close (unit=7)
! nzz4=irecnn       
       write (3,1007,rec=1)nzz4,kkll,nzz,ir,(irno(jf),jf=1,60),&
       (indic(jf),jf=1,mm)
       close (unit=3)
       end
