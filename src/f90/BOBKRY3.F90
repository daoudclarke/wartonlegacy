      program bobkry3 
!  program came from bobtran      
!     program to transpose matrix from quadratic sieve
      ...
      dimension littr(20),nar(10000,20),n(60)
      dimension ipd(50),ibase(20),jbase(20),itarg(20),jtarg(20)
...
1006  format (60i4)
      ...
      close (unit=7)
      print *,'ok2','irecnn',irecnn,'kkll',kkll
...
!      form='formatted',recl=narc9,status='old')
      kunit=1000000/(kkll*(ipd(2)+2))
      kktem=kkll/kunit
      mm=(kktem+2)*kunit
      print *,'mm',mm,'narc9',narc9
      nunit=2000000/(mm*(ipd(2)+2))
      ntem=kkll/nunit
      print *,'ntem',ntem,'nunit',nunit
      allocate (itran(nunit,mm,ipd(2)+2))
...
!      stop
      ...
      nn=kkll
      nnz2=mm*4*(ipd(2)+2)
      open (unit=2,file='ptran',access='direct',form=&
      'formatted',recl=nnz2,status='old')
      
      
      read(1,1,rec=300)kkb,ihitn,(littr(jf),jf=1,20),((nar(jk,jf),&
      jf=1,ipd(2)+2),jk=1,kkll)
      print *,kkb,ihitn,'mm',mm,'kkll',kkll
      print *,'nar6',(nar(6,jf),jf=1,nar(6,2)+2) ,'narc9',narc9
      ...
      read(1,1,rec=i)kkb,ihitn,(littr(jf),jf=1,20),((nar(jk,jf),&
      jf=1,ipd(2)+2),jk=1,kkll)
      print *,'i',i
      do jf=1,ipd(2)+2
      itran(1,i,jf)=littr(jf)
      end do
      
      do j=2,nunit
      do jf=1,ipd(2)+2
      itran(j,i,jf)=nar(j-1,jf)
      end do
      
      end do
      end do
      print *,'stop1','nnz2',nnz2,'ipd2',ipd(2)
      
      do i=1,nunit
      print *,'i2',i
      write (2,2,rec=i)((itran(i,j,jf),jf=1,ipd(2)+2),j=1,mm)
      end do
       
      do k=1,ntem-1
      print *,'k=',k,'nunit',nunit,'ntem',ntem,'mm',mm
      js=k*nunit
      do i=1,mm
      read (1,1,rec=i)kkb,ihitn,(littr(jf),jf=1,20),((nar(jk,jf),&
      jf=1,ipd(2)+2),jk=1,kkll)
      ...
      do j=1,nunit
      do jf=1,ipd(2)+2
      itran(j,i,jf)=nar(js+j-1,jf)
      end do
      end do
      end do
      do i=1,nunit
      ijs=i+js
      write (2,2,rec=ijs)((itran(i,j,jf),jf=1,ipd(2)+2),j=1,mm)
      end do
      end do
      js=nunit*ntem
