      ...
      do i=83,87
      read (2,4992,rec=i)(ipr(jf),jf=1,65000)
      print *,'ipr1=',ipr(1),'ipr2',ipr(2),'ipr65000=',ipr(65000)
      end do

      open(unit=4,file='ecmprim2',access='direct',form=&
      'formatted',recl=585000,status='old')
      do i=316,318
      read (4,5992,rec=i)(ipr(jf),jf=1,65000)
      print *,'iprlast',(ipr(jf),jf=65000,65000),'ipr1',ipr(1)
      end do
      stop
! primes have been obtained up to 500584111 : last rec. is 318
! to extend file start k is 319 and start number is 500584111+2 or
! 500584113
      do jf=1,50
      kcarr(jf)=0
      end do
...
!      
      do k=319,500
      if (mnum(1).eq.6)goto 620

!      inlen=2
!      mnum(1)=9918
!      mnum(2)=3123
      icon=1
599   call mpprime(icorp,inlen)
      if (icorp.eq.0)goto 600
      if (inlen.eq.2)goto 610
      ipr(icon)=mnum(1)*100000000+mnum(2)*10000+mnum(3)
      goto 611
610   ipr(icon)=mnum(1)*10000+mnum(2)
611   if (icon.eq.65000)goto 602
      icon=icon+1
...
600   karr(1)=0
      ...
      inlen=kcarr(2)
      goto 599
602   write(4,5992,rec=k)(ipr(jf),jf=1,65000)
      print *,'ipr65000=',ipr(65000)
...
620   close (unit=4)
      close (unit=2)
      print *,'k+1',k,'ipr65000',ipr(65000)
