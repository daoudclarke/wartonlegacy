...
!      dimension inv16(50),inv27(50),inv256(50),kcub(6),kzans(30),ksol(4) 
       ...
       dimension minv(50),mnarr(100,3,50),ipath(100),inarr(100),ipow(100)
       dimension ibase(100),itarg(100)
       ip(1)=0
       ip(2)=1
       ip(3)=13
       kard(1)=0
       ...
       print *,'minv',(minv(jf),jf=1,minv(2)+2)
...
12     ipoint=0
       itop=icurr
       print *,'mnarrs1',(mnarr(1,1,jf),jf=1,mnarr(1,1,2)+2),&
       (mnarr(1,2,jf),jf=1,mnarr(1,2,2)+2),&
       (mnarr(1,3,jf),jf=1,mnarr(1,3,2)+2),'narr1',inarr(1)
       do jf=2,ibase(2)+2
       if (ibase(jf).ne.mnarr(1,2,jf))goto 122
       end do
       print *,'target reached exponent=2'
       stop
122    do jf=2,ibase(2)+2       
       if (ibase(jf).ne.mnarr(1,3,jf))goto 123
       end do
       print *,'target reached exponent=2'
       stop
123    a=a
...
15     end do
       if (ipoint.eq.1)goto 24
       do jf=2,ipow(2)+2
       if (ipow(jf).lt.ip(jf))goto 16
       if (ipow(jf).gt.ip(jf))goto 17
       end do
       goto 17
16     a=a 
!       print *,'ipathicurr',ipath(icurr)




161    if (ipath(icurr).eq.1)goto 20
       do jf=1,mnarr(icurr,ipath(icurr),2)+2
       marr(jf)=mnarr(icurr,ipath(icurr),jf)
       end do
       do jf=1,minv(2)+2
       mbarr(jf)=minv(jf)
       end do
       ...
       call mendiv
       do jf=1,mcarr(2)+2
       mnarr(icurr+1,1,jf)=mcarr(jf)
       end do
20     do jf=1,mnarr(icurr,ipath(icurr),2)+2
       kard(jf)=mnarr(icurr,ipath(icurr),jf)
       ncom(1,jf)=kard(jf)
       end do
       call mpkron(k)
       if (k.eq.-1)goto 30
       ...
       call cornsq
       do jf=1,isqurar(1,1,2)+2
       mnarr(icurr+1,2,jf)=isqurar(1,1,jf)
       kbarr(jf)=isqurar(1,1,jf)
       end do
       do jf=1,ip(2)+2
       karr(jf)=ip(jf)
       ...
       call mpadd(1)
       do jf=1,kcarr(2)+2
       mnarr(icurr+1,3,jf)=kcarr(jf)
       end do
       if (ipath(icurr).eq.1)goto 22
       inarr(icurr+1)=3
       ipath(icurr+1)=1
       goto 23
22     inarr(icurr+1)=2
...
23     icurr=icurr+1
       do jf=2,ibase(2)+2
       if (ibase(jf).ne.mnarr(icurr,2,jf))goto 25
       end do
       ipoint=1
       itop=icurr-1
       goto 121
24     print *,'target reached exponent=',(ipow(jf),jf=1,ipow(2)+2)
       print *,'ipath',(ipath(jf),jf=1,icurr)
       stop
25     do jf=2,ibase(2)+2
       if (ibase(jf).ne.mnarr(icurr,3,jf))goto 26
       end do
       ipoint=1
       itop=icurr-1
       goto 121
26     icon=icon+inarr(icurr)
       goto 12
30     if (inarr(icurr).eq.1)goto 17
       if (ipath(icurr).eq.1)goto 37
       inarr(icurr+1)=1
       ipath(icurr+1)=1
       icurr=icurr+1
       icon=icon+inarr(icurr)
       goto 12
17     icurr=icurr-1
37     a=a
!37     print *,'ipathicurr',ipath(icurr),'inarric',inarr(icurr),&  
!       'icurr',icurr,'ipath',(ipath(jf),jf=1,icurr)
!       do jk=1,icurr
!       print *,'jk',jk,'mn1',(mnarr(jk,1,jf),jf=1,mnarr(jk,1,2)+2),&
!       'mmn2',(mnarr(jk,2,jf),jf=1,mnarr(jk,2,2)+2),&
!       'mn3',(mnarr(jk,3,jf),jf=1,mnarr(jk,3,2)+2),'inarr',inarr(jk)
!       end do
       
       if ((ipath(icurr).eq.2).and.(inarr(icurr).eq.2))goto 50
       if (ipath(icurr).lt.inarr(icurr))goto 50
       icurr=icurr-1
       if (icurr.eq.0)goto 99
       goto 37
50     ipath(icurr)=ipath(icurr)+1 
!       print *,'icon',icon,'icurr',icurr,'inarricurr',inarr(icurr)
       goto 161
99     print *,'major error'
