...
!     idis has positive value of negative discriminant      
      ...
      common iarq(2)
      common jay(2,620),ipi(420),icurr(2000) 
      common mult1(7,2,600),mult2(7,2,600),mult3(7,2,600)
...
!    last one run was modjpol5      
!!!      open (unit=3,file='modjlen6',access='direct',form=&
!!!      'formatted',recl=600,status='old')
!!!      read (3,1005,rec=1)limt
!!!      open (unit=2,file='modjpol6',access='sequential')
!!!      read (2,*)(irecarr(jf),jf=1,limt)
      irecarr(1)=2
      irecarr(2)=23
      irecarr(3)=3
      irecarr(4)=22
      ...
      irecarr(22)=9375
      limt2=22
!!!      write (2,*)(irecarr(jf),jf=1,limt2)
!!!      write (3,1005,rec=1)limt,limt2
!!!      close(unit=2)
!!!      close(unit=3)
!!!      stop
...
!    below not considered
      ...
      idisarr(13)=883
      idisarr(14)=907
! successes already found are 379 and 643 both final
! tried 124 139 219 259 291 323 355 475 568 592 667 723 763 772      
! tried 39 55 56 63 68 136 155 171 203 208 955 1003 1027 1227 1243 1387
! tried 1411 1507 1555 955 not ok
! successes are 259 and 291 203 955 1027 1411     
! finals 379 643 291 203 1027 1411 259 and 331
! all discriminants for degrees 1 2 3 4 have been tried      
! below degree 5 discriminants
! semi-successes degree five 131 179 227 316 347 443     
! complete success degree five 619 , 1867 ok but run on bobmodj3      
! change to degree six discriminants     
! no successes degree six 207 to 472 inclusive  or 628 to 1192 inc    
! 531 and 1315 and 1603 successful(degree 6)  1843 successful but bobmodj3    
! had to be used
      idisarr(1)=87
!     idisarr(1)=207
      idisarr(2)=104
...
!      idis=8
      ...
      rtd3=sqrt(ddis/3)
      irtd3=rtd3
      print *,'irtd3',irtd3,'idis',idis,'iffsw',iffsw
      ...
      iia=1
      ibpar=iib*(-1)
!      if (idis.ne.15)goto 200
!      ibpar=17
!      iia=19
!      if (idis.ne.7)goto 200
!      ibpar=9
!      iia=11
!       if (idis.ne.8)goto 200
!       ibpar=14
!       iia=11
!       if (idis.ne.23)goto 200
!       ibpar=-1
!       iia=2
!       ibpar=25
!       iia=27
200   print *,'iit',iit,'ibpar',ibpar,'iia',iia,'isuscon',isuscon
      if (isuscon.eq.0)goto 204
      print *,'successes',(isold(jf),jf=1,isuscon)
204   a=a      
      iendic=1
      print *,'fbok'
...
140   iendic=2
      ibpar=iib*(-1)
      katemp=iia
!      if (idis.ne.15)goto 201
!      ibpar=15
!      iia=20
!      if (idis.ne.23)goto 201
!      ibpar=19
!      iia=24
      print *,'ibpar',ibpar
201   call submodj(iendic,iia,ibpar,idis)
      iia=katemp
...
10    lenjay1=jay(1,2)
      ...
      print *,'test conjugate','ibpar',ibpar,'iia',iia,'idis',idis
      call submodj(iendic,iia,ibpar,idis)
      
      if ((jay(1,2).ne.lenjay1).or.(jay(1,3).ne.iprej1))goto 120
      if ((jay(2,2).ne.lenjay2).or.(jay(2,3).ne.iprej2))goto 120
      if (iffsw.eq.0)goto 206
      isuscon=isuscon+1
      print *,'to be investigated',idis
      isold(isuscon)=idis
      goto 112
206   do jf=1,jay(1,2)+2
      marr(jf)=jay(1,jf)
      end do
      print *,'jay1',(jay(1,jf),jf=1,10)
      print *,'jay2',(jay(2,jf),jf=1,10)
!      if (jay(2,2).gt.300)goto 120
      do ii=1,idegm+1
      do ibig=1,2
...
20    iia=iia+1
      ...
      mult1(i,ibig,2)=mult1(i,ibig,2)-400
...
15    return
      ...
      common mult1(7,2,600),mult2(7,2,600),mult3(7,2,600)
      common marr(2000),mbarr(2000),mcarr(2000),mdarr(2000)
      dimension icomp1(600),icomp2(600)
...
!     program for computing modular invariants ex "bobtrial"
      ...
      dimension ipre(3000)
      dimension ipisum(3,620),iptop(3,620)
      dimension itop1(720),itop2(720),ifack(620),iesum(620)
      dimension iiq(2,620),idel(2,2,1000),itab(600),iffy(2,620)
      dimension ieesum(2,600),itemsy(620),ifsum(620)
      dimension igsum(620),itemst(2,620),ittop(2,620),inorm(2,1000)
      dimension iden(620),ntop(2,420),ieesum2(2,1000)
      iib=ibpar
      ...
      do jf=iptop(k,2)+1,iptop(k,2)+400
      karr(jf)=0
      end do
...
732   print *,'int. root',(icurr(jf),jf=1,100)
      ...
      goto 200
740   print *,'not perfect square','iia',iia
      print *,'len',icurr(2),'icurr3',icurr(3),'icurr4',icurr(4)
...
!     provision for iia.ne.1      
      ...
      itop1(2)=icont
      itemsy(2)=icont
      print *,'itop1',(itop1(jf),jf=1,100),'lenipi',ipi(2),'icurl',icurr(2)
...
!      if (icont.le.200)goto 1000
      if (icont.le.400)goto 621
      goto 622
621   kbarr(1)=0
      kbarr(2)=0
      goto 623
622   do jf=1,icont-400
      kbarr(jf+2)=ipqt(jf)
      end do
      kbarr(1)=0
623   kbarr(2)=icont-400
      do jf=1,iesum(2)+2
      karr(jf)=iesum(jf)
      end do
      call mpadd(0)
...
!      itop1(2)=ilen3
      ...
      inabc=101
!      if (icont.le.200)goto 1000
      if (icont.le.400)goto 611
      goto 612
611   kbarr(1)=0
      kbarr(2)=0
      goto 613
612   do jf=1,icont-400
      kbarr(jf+2)=ipqt(jf)
      end do
...
!      end do
      ...
      inabc=102
!      if (icont.le.200)goto 1000
      if (icont.le.400)goto 631
      goto 632
631   kbarr(1)=0      
      kbarr(2)=0
      goto 633
632   do jf=1,icont-400
      kbarr(jf+2)=ipqt(jf)
      end do
      kbarr(1)=mod(i,2)
      kbarr(2)=icont-400
